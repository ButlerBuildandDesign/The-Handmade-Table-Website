...
/* ═══ PORTFOLIO: TABLES — TRESTLE TABLE PHOTOS ═══════════════ */
const TRESTLE_TABLE_R2 = 'https://pub-783c0f87e6f341c197ff7ad4188ba57e.r2.dev/Portfolio_Tables_TR/';
const trestleTableImages = Array.from({ length: 57 }, (_, i) =>
  TRESTLE_TABLE_R2 + 'Portfolio_Tables_TR' + (i + 1) + '.jpeg'
);

function renderTrestleTableGrid() {
  const grid = document.getElementById('trestle-table-grid');
  if (!grid || grid.childElementCount) return;
  grid.innerHTML = trestleTableImages.map((src, i) =>
    `<div class="portfolio-photo" onclick="openGalleryLightbox(trestleTableImages, ${i})">
      <img src="${src}" alt="Trestle Table" loading="lazy">
    </div>`
  ).join('');
}
...
/* ═══ PORTFOLIO: TABLES — TRADITIONAL TABLE PHOTOS ═══════════ */
const TRADITIONAL_TABLE_R2 = 'https://pub-783c0f87e6f341c197ff7ad4188ba57e.r2.dev/Portfolio_Tables_TT/';
const traditionalTableImages = Array.from({ length: 4 }, (_, i) =>
  TRADITIONAL_TABLE_R2 + 'Portfolio_Tables_TT' + (i + 1) + '.jpeg'
);

function renderTraditionalTableGrid() {
  const grid = document.getElementById('traditional-table-grid');
  if (!grid || grid.childElementCount) return;
  grid.innerHTML = traditionalTableImages.map((src, i) =>
    `<div class="portfolio-photo" onclick="openGalleryLightbox(traditionalTableImages, ${i})">
      <img src="${src}" alt="Traditional Table" loading="lazy">
    </div>`
  ).join('');
}
...
function renderArchElementsGrid() {
  ...
  if (!grid || grid.childElementCount) return;
  grid.innerHTML = archElementsImages.map((src, i) =>
    `<div class="portfolio-photo" onclick="openGalleryLightbox(archElementsImages, ${i})">
      <img src="${src}" alt="Architectural Elements" loading="lazy">
    </div>`
  ).join('');
}
...
function showPage(page, anchorId) {
  ...
  const navMap = {
    ...
    'portfolio-tables-traditional': 'nav-portfolio',
  ...
  if (page === 'portfolio-tables-traditional') renderTraditionalTableGrid();
